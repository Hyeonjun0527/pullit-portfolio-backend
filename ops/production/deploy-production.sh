...
docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" run --rm --no-deps pullit-prod-storage-init
docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" up -d --no-deps pullit-prod-app pullit-prod-worker
for attempt in {1..45}; do
  worker_id="$(docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" ps -q pullit-prod-worker)"
  if docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" exec -T pullit-prod-app wget --quiet --spider http://localhost:8080/actuator/health \
    && [ -n "$worker_id" ] \
    && [ "$(docker inspect --format '{{.State.Health.Status}}' "$worker_id" 2>/dev/null || true)" = healthy ]; then
